...
$categorias = \common\models\Categoria::find()
    ->all();
...
$linhasCarrinho = $carrinho ? \common\models\Linhacarrinho::find()
...
<?php
$this->registerCssFile('@web/css/vendor.css');
$this->registerCssFile('@web/css/site.css');
$this->registerCssFile('@web/css/normalize.css');
$this->registerCssFile('@web/css/artigosIndex.css');
    ...
    <div class="offcanvas-body">
        <div class="order-md-last">
            <h4 class="d-flex justify-content-between align-items-center mb-3">
                <span class="text-primary">O seu Carrinho</span>
                <span class="badge bg-primary rounded-pill"><?= count($linhasCarrinho) ?></span> <!-- Quantidade de itens no carrinho -->
            </h4>
            <ul class="list-group mb-3">
                <?php if (!empty($linhasCarrinho)): ?>
                    <?php foreach ($linhasCarrinho as $linha): ?>
                        <?php
                        // Caminho da imagem do artigo
                        $imagemSrc = Yii::getAlias('@web') . '/imagens/materiais/' . $linha->artigo->imagem;
                        ?>
                        <li class="list-group-item d-flex justify-content-between lh-sm">
                            <div class="d-flex">
                                <!-- Imagem do artigo -->
                                <img src="<?= $imagemSrc ?>" alt="<?= Html::encode($linha->artigo->nome) ?>" class="img-fluid" style="width: 50px; height: 50px; object-fit: cover; margin-right: 10px;">

                                <div>
                                    <h6 class="my-0"><?= Html::encode($linha->artigo->nome) ?></h6>
                                    <small class="text-body-secondary"><?= Html::encode($linha->artigo->descricao) ?></small>
                                    <br>
                                    <small class="text-body-secondary"><?= Html::encode($linha->quantidade)?>x</small>
                                </div>
                            </div>
                            <span class="text-body-secondary"><?= Html::encode(number_format($linha->artigo->precoFinal, 2)) ?> €</span>
                        </li>
                    <?php endforeach; ?>
                    <li class="list-group-item d-flex justify-content-between">
                        ...
                        <strong>
                            ...
                            echo number_format($carrinho->valorTotal, 2);
                            ?> €
                        </strong>
                    </li>
                <?php else: ?>
            ...
            <div class="d-flex justify-content-between">
                <a href="<?= Yii::$app->urlManager->createUrl(['carrinho-compras/index']) ?>"
                   class="btn btn-primary btn-lg">
                    Ver e editar carrinho
                </a>
                <a href="<?= Yii::$app->urlManager->createUrl(['carrinho-compras/checkout']) ?>"
...
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasNavbar">
    <div class="offcanvas-header justify-content-between">
        <h4 class="fw-normal text-uppercase fs-6">Categorias</h4>
        <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
        ...
        <ul class="navbar-nav justify-content-end menu-list list-unstyled d-flex gap-md-3 mb-0">
            <?php foreach ($categorias as $categoria): ?>
                <li class="nav-item border-dashed">
                    <a href="<?= Yii::$app->urlManager->createUrl(['artigos/artigos', 'id' => $categoria->id]) ?>" class="nav-link d-flex align-items-center gap-3 text-dark p-2">

                        <span><?= Html::encode($categoria->descricao) ?></span>
                    </a>
                </li>
            <?php endforeach; ?>
        </ul>
        ...
        <div class="row py-3 border-bottom">
            ...
            <div class="col-lg-4">
                <ul class="navbar-nav list-unstyled d-flex flex-row gap-3 gap-lg-5 justify-content-center flex-wrap align-items-center mb-0 fw-bold text-uppercase text-dark">
                    <li class="nav-item active">
                        <a href="index.html" class="nav-link">Configurador</a>
                    </li>
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle pe-3" role="button" id="pages" data-bs-toggle="dropdown" aria-expanded="false">Páginas</a>
                    ...
                    <li>
                        <button id="theme-toggle" class="btn btn-primary">Alternar Tema</button>
            ...
            <div class="col-sm-8 col-lg-2 d-flex gap-5 align-items-center justify-content-center justify-content-sm-end">
                <ul class="d-flex justify-content-end list-unstyled m-0">
                    ...
                    <li>
                        <a class=""
                           href="<?= Yii::$app->urlManager->createUrl(['favoritos/index']) ?>">
                        <svg width="24" height="24"><use xlink:href="#wishlist"></use></svg>
                        </a>
                    </li>
                    <li>
                        <a href="#" class="p-2 mx-1" data-bs-toggle="offcanvas" data-bs-target="#offcanvasCart" aria-controls="offcanvasCart">
                            <svg width="24" height="24"><use xlink:href="#shopping-bag"></use></svg><?= $cartItemCount ?>
                        </a>
                    </li>
                </ul>
            </div>
...
<script src="/frontend/web/js/jquery-1.11.0.min.js"></script>
...
<script src="/frontend/web/js/darkmode.js"></script>
<script>
